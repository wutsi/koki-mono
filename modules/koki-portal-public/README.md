...
- **Redis**: Redis server for caching (required for production)
- **RabbitMQ** (optional for event tracking)
...
- `RABBITMQ_URL`: RabbitMQ connection URL (if using event tracking)

#### Redis Configuration

Redis is required for production caching:
...
```bash
mvn clean package
java -jar target/koki-portal-public.jar
```

The portal will start on `http://localhost:8082`.

#### Access the Portal

Once running, access the public portal in your web browser:

```
http://localhost:8082
```

**Public Access**: No authentication required for viewing listings.
...
```
web: java $JAVA_OPTS -Dserver.port=$PORT -jar koki-portal-public.jar --spring.profiles.active=$APP_PROFILE
```
...
The portal includes Selenium-based UI tests (14 test files). To run integration tests:
