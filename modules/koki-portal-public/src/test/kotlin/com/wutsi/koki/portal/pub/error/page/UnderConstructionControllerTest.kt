package com.wutsi.koki.portal.pub.error.page

import com.nhaarman.mockitokotlin2.any
import com.nhaarman.mockitokotlin2.doReturn
import com.nhaarman.mockitokotlin2.eq
import com.nhaarman.mockitokotlin2.whenever
import com.wutsi.koki.portal.pub.AbstractPageControllerTest
import com.wutsi.koki.portal.pub.TenantFixtures.tenants
import com.wutsi.koki.portal.pub.common.page.PageName
import com.wutsi.koki.tenant.dto.SearchTenantResponse
import com.wutsi.koki.tenant.dto.TenantStatus
import org.springframework.http.HttpStatus
import org.springframework.http.ResponseEntity
import kotlin.test.Test

class UnderConstructionControllerTest : AbstractPageControllerTest() {
    @Test
    fun suspended() {
        doReturn(
            ResponseEntity(
                SearchTenantResponse(
                    tenants = tenants.map { tenant ->
                        tenant.copy(status = TenantStatus.NEW, clientPortalUrl = "http://localhost:$port")
                    }
                ),
                HttpStatus.OK,
            )
        ).whenever(restWithoutTenantHeader)
            .getForEntity(
                any<String>(),
                eq(SearchTenantResponse::class.java)
            )

        navigateTo("/")

        assertCurrentPageIs(PageName.ERROR_UNDER_CONSTRUCTION)
    }
}
