...
- **Access to koki-server**: A running instance of koki-server (locally or remote)
- **Authentication Credentials**: Valid tenant ID and user credentials for authentication

### Installation

Add the koki-sdk dependency to your Maven project:
...
**Note**: The SDK requires the following dependencies (marked as `provided` scope):

- `spring-web` (version 6.x)
...
Create a Spring configuration class to set up the SDK clients:
...
class KokiSdkConfiguration {
    ...
    // Add other domain clients as needed...
}
```

#### RestTemplate Configuration with Headers

Configure `RestTemplate` to automatically include tenant ID and authentication headers:
...
class KokiSdkConfiguration(
    ...
    fun restTemplate(): RestTemplate {
        ...
        restTemplate.interceptors.add(HeaderInterceptor(tenantProvider, accessTokenHolder))
        return restTemplate
    }
}
...
class HeaderInterceptor(
    ...
    override fun intercept(
        ...
        request.headers[HttpHeaders.AUTHORIZATION] = "Bearer ${accessTokenHolder.get()}"
...
Once configured, inject and use the SDK clients:
...
class MyKokiService(
    ...
    fun authenticateAndGetAccounts() {
        // 1. Authenticate
        ...
        println("Found ${accounts.accounts.size} accounts")
    }
}
```
...
@Service
class FileUploadService(private val kokiFiles: KokiFiles) {

    fun uploadFile(file: MultipartFile, accountId: Long) {
        val response = kokiFiles.upload(
            ownerId = accountId,
        ...
        println("File uploaded with ID: ${response.id}")
    }
}
```
...
The SDK currently does not include test files, but you can test the SDK integration in your application:

```bash
# Run tests in your application that uses the SDK
mvn test
```

For integration testing with a real koki-server instance, ensure the server is running and accessible at the configured
base URL.
