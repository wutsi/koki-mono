...
class CqsBatchService(
    ...
    fun computeAll(tenantId: Long) {
        ...
        try {
            ...
            logger.add("cqs_batch_total_processed", totalProcessed)
            logger.add("cqs_batch_total_updated", totalUpdated)
            logger.add("cqs_batch_success", true)
        } catch (ex: Exception) {
            logger.add("cqs_batch_success", false)
            logger.add("cqs_batch_error", ex.message)
            throw ex
        }
    }
}
