...
interface ConfigurationRepository : CrudRepository<ConfigurationEntity, Long> {
    @Query("SELECT C FROM ConfigurationEntity C WHERE C.attribute.tenantId=?1 ORDER BY C.attribute.name")
    fun findByTenantId(tenantId: Long): List<ConfigurationEntity>

    @Query("SELECT C FROM ConfigurationEntity C WHERE C.attribute.tenantId=?1 AND C.attribute.name IN  ?2")
