...
- valid: Whether the listing is valid or not. (boolean - Default value is `true`)
...
- price: Total sale or rental price (integer).
    ...
    - For rental properties, we support only monthly prices. Reject listing with daily or weekly prices.
    - If the price is provided per square meter, multiply it by the lotArea to get the total price
- revenue: The total monthly revenue generated by the property (integer).
    - If the revenue is a range, use the maximum value of the range.
...
- street: Address of the property - Name of the street. IMPORTANT: Do not pick the street from the agent address!
...
- numberOfSigners: Number of signers. If mentioned as `multiple`, ignore it (integer)
- mutationType: Type of mutations. Values can be {{mutationTypes}}
- transactionWithNotary: `true` when transaction is performed with a notary (boolean)
...
- Phone: {{agentPhone}}
- Address:
    - Street: {{agentAddressStreet}}
    - City: {{agentAddressCity}}
    - Country: {{agentAddressCountry}}
