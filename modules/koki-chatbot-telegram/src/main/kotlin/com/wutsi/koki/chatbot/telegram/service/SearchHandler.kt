...
class TelegramConsumer(
    ...
    private fun sendViewMoreCTA(data: SearchAgentData, tenant: TenantModel, update: Update) {
        val cta = InlineKeyboardButton("View More Properties...")
        cta.url = viewMoreCTAUrl(data, tenant, update)

        val location = (data.searchParameters.neighborhood ?: data.searchParameters.city)
            ?.let { loc -> StringUtils.toAscii(loc).lowercase() }
            ?: ""

        val msg = SendMessage(update.message.chatId.toString(), "View more properties in $location")
    ...
    private fun viewMoreCTAUrl(data: SearchAgentData, tenant: TenantModel, update: Update): String {
        val language = update.message.from.languageCode
