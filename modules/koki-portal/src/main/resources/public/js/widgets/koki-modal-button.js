 ...
 *   data-modal-title: Title of the modal
 */
class ModalButtonWidget {
    init() {
        let count = 0;
        document.querySelectorAll('[data-component-id=modal-button]')
            .forEach((elt) => {
                    count++;
                    elt.removeEventListener('click', koki.widgets.modalButton._on_click);
                    elt.addEventListener('click', koki.widgets.modalButton._on_click);
                }
            );
    ...
    _on_click() {
        ...
        koki.widgets.modal.open(url, title);
    }
}
                    ...
                    if (response.ok) {
                        response.text()
                            .then(html => {
                                ...
                                const btnClose = document.querySelector("#koki-modal-body .btn-modal-close");
                                if (btnClose) {
                                    btnClose.addEventListener('click', koki.widgets.modal.close);
                                }
