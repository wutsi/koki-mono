...
class AgentController(
        ...
        val markers = listings
            ...
            .map { listing ->
                mapOf(
                    ...
                    "location" to listOf(listing.address?.neighbourhood?.name, listing.address?.city?.name)
                        .filterNotNull()
                        .joinToString(", "),
    ...
    fun toMapCenterPoint(listings: List<ListingModel>): LocationModel? {
        ...
        return sorted.filter { location -> location.type == LocationType.NEIGHBORHOOD }.firstOrNull()
            ?: sorted.firstOrNull()
    }
}
