...
- Access to Koki Server API (backend REST API)
- Chromium browser (for Selenium tests)
...
Configure the application by setting the following environment variables or
updating `src/main/resources/application.yml`:

**Server Configuration:**

- `SERVER_PORT`: Server port (default: `8081`)
- `APP_PROFILE`: Spring profile to activate (e.g., `local`, `test`, `prod`)

**Backend API Configuration:**

- `KOKI_SERVER_URL`: URL of the Koki Server backend API (e.g., `http://localhost:8080`)
- `KOKI_SERVER_API_KEY`: API key for authenticating with the backend server

**Cache Configuration:**

- `REDIS_URL`: Redis connection URL for session management and caching
...
- `SPRING_SERVLET_MULTIPART_MAX_FILE_SIZE`: Maximum file upload size (default: `50MB`)
- `SPRING_SERVLET_MULTIPART_MAX_REQUEST_SIZE`: Maximum request size (default: `50MB`)
...
The portal will start on `http://localhost:8081`. Navigate to the home page to access the application.
