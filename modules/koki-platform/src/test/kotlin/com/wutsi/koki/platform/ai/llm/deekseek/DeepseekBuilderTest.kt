...
class DeepseekBuilderTest {
    val config = mapOf(
        ConfigurationName.AI_PROVIDER_DEEPSEEK_MODEL to "model-xyz",
        ConfigurationName.AI_PROVIDER_DEEPSEEK_API_KEY to "sk-43904309340"
    )
    ...
    fun build() {
        ...
        assertTrue(service is Deepseek)
    }

    @Test
    fun missingModel() {
        assertThrows<LLMNotConfiguredException> {
            builder.build(config.filter { entry -> entry.key != ConfigurationName.AI_PROVIDER_DEEPSEEK_MODEL })
        }
    }
    ...
    fun missingApiKey() {
        assertThrows<LLMNotConfiguredException> {
            builder.build(config.filter { entry -> entry.key != ConfigurationName.AI_PROVIDER_DEEPSEEK_API_KEY })
        }
    }
}
